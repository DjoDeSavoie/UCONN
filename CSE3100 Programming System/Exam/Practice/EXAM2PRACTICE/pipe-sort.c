...
void pipe_sort(int seed, int n, int print_sorted, int num_printed){
    ...
    int child1 = fork();
    if(child1 == -1) die("fork() for child 1");  // Check for fork error

    if (child1 == 0) {
        // Child 1 process
        close(pd1[PFD_READ]); // Close read end of pipe 1

        // Sort the first half
        qsort(a, half, sizeof(int), compare_int);

        // Write sorted data to pipe
        for (int i = 0; i < half; i++) {
            write_int(pd1[PFD_WRITE], a[i]);
        }

        close(pd1[PFD_WRITE]); // Close write end of pipe 1
        exit(EXIT_SUCCESS);
    }

    int child2 = fork();
    if(child2 == -1) die("fork() for child 2");  // Check for fork error

    if (child2 == 0) {
        // Child 2 process
        close(pd2[PFD_READ]); // Close read end of pipe 2
        ...
        close(pd2[PFD_WRITE]); // Close write end of pipe 2
        exit(EXIT_SUCCESS);
    }

    // Parent process
    close(pd1[PFD_WRITE]); // Close write end of pipe 1 in parent
    close(pd2[PFD_WRITE]); // Close write end of pipe 2 in parent
