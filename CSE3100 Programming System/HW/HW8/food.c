...
{   
    ...
    while(!done)
    {
        ...
        // Add item to the buffer in the current column
        add_to_buffer(item, col, q);

        // Move to the next column for the next item
        col = (col + 1) % 3;

        // Increment the total items produced by this producer
        my_data->total++;

        // Check if the producer has reached its production limit
        if (my_data->total >= q->remain) {
            done = 1;
        } else {
            item++;  // Increment item for the next production cycle
        }
    }
