...
{
    // Normalize the x and y coordinates to a range of [0, 2k]
    int normalized_x = (x + k) % (2 * k + 1);
    int normalized_y = (y + k) % (2 * k + 1);

    // Combine the normalized coordinates in a way that minimizes collisions
    return (normalized_x * (2 * k + 1)) + normalized_y;
}
...
{
    node *nodeSpecificHost = malloc(sizeof(node));
    nodeSpecificHost->host = host;
    nodeSpecificHost->next = NULL;
    return nodeSpecificHost;
}
...
{
	node * visitingNode = *head;
	node * tmp = NULL;

	while (visitingNode != NULL)
	{
		tmp = visitingNode->next;
		free(visitingNode);
		visitingNode = tmp;
	}
...
int location_match(node *head, THost host)
{
	...
	while (visitingNode != NULL)
	{
		if(visitingNode->host.x == host.x && visitingNode->host.y == host.y){
			return 1;
		}
...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
    ...
    for (int i = 0; i < n_arr; i++) {
        remove_all(&(p_arr[i])); // Clear each linked list
    }
	...
	for(int i = 0; i < m; i++)
	{
		...
		//we do the case when a host get out of the map : 
		if(hosts[i].x > k) hosts[i].x = -k;
		if(hosts[i].x < -k) hosts[i].x = k;
		if(hosts[i].y > k) hosts[i].x = -k;
		if(hosts[i].y < -k) hosts[i].x = k;
