...
void start_program(Program *programs, int num_programs, int cur)
{
    ...
    if (pid < 0) {
        ...
        for (int i = 0; i < num_programs; i++) {
            if (programs[i].fd_in != -1) close(programs[i].fd_in);
            if (programs[i].fd_out != -1) close(programs[i].fd_out);
        }
        ...
        // Close the write-end of the current pipe (no longer needed by parent)
        if (programs[cur].fd_out != -1) {
            close(programs[cur].fd_out);
        }

        // Close the read-end of the previous pipe (no longer needed by parent)
        if (cur > 0 && programs[cur].fd_in != -1) {
...
int wait_on_program(Program *prog)
{
    ...
    if (WIFEXITED(exitStatus)) {
        return WEXITSTATUS(exitStatus); // Return the exit status of the child
    } else {
        return -1; // If the process didn't exit normally
    }
}
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    for (int i = 0; i < num_programs - 1; i++)
    {
        ...
        programs[i].fd_out = pipefd[PIPEFD_WRITE];   // Output of the current program
        programs[i + 1].fd_in = pipefd[PIPEFD_READ]; // Input of the next program
    }
}
