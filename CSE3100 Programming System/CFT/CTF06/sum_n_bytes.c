...
unsigned sum_n_bytes(unsigned *p, int n)
{
    ...
    unsigned sum = 0;
    // Cast the pointer p to an unsigned char pointer with the size of a byte so we can access each byte by incrementing with i in the loop
    unsigned char *bytePtr = (unsigned char *)p;
    
    // Loop over the first n bytes
    for (int i = 0; i < n; i++) {
        sum += bytePtr[i];  // Access each byte and add it to the sum
    }

    return sum;  // Return the unsigned sum of the bytes
