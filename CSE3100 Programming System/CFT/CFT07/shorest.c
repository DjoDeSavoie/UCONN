...
t_pair *shortest_dist_pair(int n, t_point points[])
{
	...
	double d;
	t_pair *p_pair;
	p_pair = (t_pair*)malloc(sizeof(t_pair));
	if(p_pair == NULL)
	{
    ...
    for(int i = 0; i < n; i++){
        for(int j = i+1; j < n; j++){ //we go through all the combinations of points
            double dist = squared_dist(&(points[i]), &points[j]); //compute the distance
            if(dist < min_d){ //if the computed distance is less than the min distance
